      ...
      <AnimatePresence>
        {showContextMenu && !isSelectionMode && (
          <>
            ...
            <motion.div
              initial={{ opacity: 0, scale: 0.92, y: openUpwards ? 10 : -10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.92, y: openUpwards ? 10 : -10 }}
              transition={{ type: 'spring', damping: 25, stiffness: 350 }}
              className={`absolute z-50 min-w-[220px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/90 rounded-2xl shadow-2xl p-2 flex flex-col gap-1.5 text-xs backdrop-blur-xl ${
                openUpwards
                  ? 'bottom-full mb-2'
                  : 'top-full mt-2'
              } ${isMe ? 'right-0' : 'left-0'}`}
            >
              {/* Emoji Reactions Bar */}
              <div className="flex items-center justify-between gap-1 p-1 bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl overflow-x-auto scrollbar-none">
                {REACTION_EMOJIS.map((emoji) => (
                  <button
                    key={emoji}
                    onClick={() => handleSelectReaction(emoji)}
                    className="text-base hover:scale-125 transition-transform p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-800"
                    title={`React ${emoji}`}
                  >
                    {emoji}
                  </button>
              ...
              {/* Menu Title Header */}
              <div className="px-2 py-1 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-[11px] font-semibold text-slate-500 dark:text-slate-400">
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-medium text-left"
              >
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-medium text-left"
              >
                <Pin className="w-4 h-4 text-amber-500 dark:text-amber-400 shrink-0" />
              ...
              {message.text && (
                <button
                  ...
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-medium text-left"
                >
                  {copied ? <Check className="w-4 h-4 text-emerald-500 shrink-0" /> : <Copy className="w-4 h-4 text-cyan-600 dark:text-cyan-400 shrink-0" />}
                  <span>Copy Text</span>
                </button>
              )}
              ...
              {(isMe || isAdmin) && (
                <button
                  ...
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/70 transition-colors font-medium text-left border-t border-slate-200 dark:border-slate-800/80 mt-0.5 pt-1.5"
                >
